...
Ввод: """))
	...
	if(menu == 1):
		...
		input_name = input("Введите username пользователя (user1, user2, user3): ")
		...
		if(int(input("Запустить ребалансировку портфеля пользователя? \n1 - Да\nEnter: "))):
			...
			if(user1['type'] == 1):
				...
				if(i//1000 > 0 and i > min_different): # ~1000 - цена облигаций  ОФЗ и многих других
					print("Закупка " + str(i//1000) + " облигаций ОФЗ обеспечит диверсификацию портфеля, снизит риски и позволит поднять аллокацию облигаций до рекомендуемого уровня ~80%.")
					print("К покупке рекомендованы облигации государственного типа, т.к. гарантия сбережения денег доказывается при каждом кризисе. Только государственные облигации не упали в цене, а наоборот продолжили расти в течение кризисов последних лет.")
					...
					if(i > min_different):
						...
						if(len(low_risk) > 0):
							print("В соответствии с вашим риск-профилем рекомендуем")
							print(*low_risk)
							print("Поскольку данные акции обладают большой капитализацией и низкой волатильностью.")
						elif(max_target > 0):
				...
				if(i//1000 > 0 and i > min_different): # ~1000 - цена облигаций ОФЗ и многих других
					print("Закупка " + str(i//1000) + " облигаций ОФЗ обеспечит диверсификацию портфеля, снизит риски и позволит поднять аллокацию облигаций до рекомендуемого уровня ~50%.")
					print("К покупке рекомендованы облигации государственного типа, т.к. гарантия сбережения денег доказывается при каждом кризисе. Только государственные облигации не упали в цене, а наоборот продолжили расти в течение кризисов последних лет.")
					print("Также можно продать какие-либо акции на " + str(i) + " руб.")
				...
				if(i//1000 > 0 and i > min_different): # ~1000 - цена облигаций ОФЗ и многих других
					print("Закупка " + str(i//1000) + " облигаций ОФЗ обеспечит диверсификацию портфеля, снизит риски и позволит поднять аллокацию облигаций до рекомендуемого уровня ~20%.")
					print("К покупке рекомендованы облигации государственного типа, т.к. гарантия сбережения денег доказывается при каждом кризисе. Только государственные облигации не упали в цене, а наоборот продолжили расти в течение кризисов последних лет.")
					print("Также можно продать какие-либо акции на " + str(i) + " руб.")
